    ...
    <div class="w-full">
        ...
        <div class="flex flex-col pr-4 md:mr-4 pb-[120px]">
            ...
            <div class="md:flex md:flex-row md:justify-between my-4">
                ...
                <div>
                    <x-link-add :message="__('Add product')" :href="route('products.create')"/>
                </div>
            </div>
                    ...
                    <table class="w-full bg-white rounded-lg shadow-lg my-5">
                        <thead class="text-white">
                            <tr class="bg-teal-400 table-row rounded-none mb-0">
                                ...
                                <th scope="col" class="text-sm md:text-base p-3 text-left">SKU</th>
                                <th scope="col" class="text-sm md:text-base p-3 text-left">Name</th>
                                <th scope="col" class="text-sm md:text-base p-3 text-left">Price</th>
                    ...
                    <tbody class="flex-1 flex-none">
                        @foreach ($products as $item)
                            <tr class="flex flex-col flex-no wrap table-row mb-0">
                                ...
                                <td class="text-sm md:text-base border-grey-light border hover:bg-gray-100 p-1 md:p-3 ">{{ $item->sku }}</td>
                                <td class="text-sm md:text-base border-grey-light border hover:bg-gray-100 p-1 md:p-3 ">{{ $item->name }}</td>
                                <td class="text-sm md:text-base border-grey-light border hover:bg-gray-100 p-1 md:p-3 ">{{ $item->price }}</td>
                                <td class="text-sm md:text-base border-grey-light border hover:bg-gray-100 p-1 md:p-3 ">
                                        <form action="{{ route('products.destroy') }}" method="POST">
                                            <a class="block sm:w-full sm:inline text-sm md:text-base p-1 px-3 bg-sky-200 rounded mr-2 mb-2" href="{{ route('products.show',$item->id) }}">Show</a>
                                            <a class="block sm:w-full sm:inline text-sm md:text-base p-1 px-3 bg-emerald-200 rounded mr-2 mb-2" href="{{ route('products.edit',$item->id) }}">Edit</a>
